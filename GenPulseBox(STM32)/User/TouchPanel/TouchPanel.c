...
* Attention		 : None
*******************************************************************************/
void TouchPanel_Calibrate(Coordinate *screenSample)
{
  uint8_t i;
  Coordinate * Ptr;
  ...
  {
   LCD_Clear(clBlack);
   GUI_Text(44, 10, "Touch crosshair to calibrate", clRed, clWhite);
   delay_ms(300);
...
void TouchPanel_SettCalibrate(Coordinate *screenSample) {
  uint8_t i;

  for(i=0;i<3;i++) {
		ScreenSample[i].x= screenSample[i].x; ScreenSample[i].y= screenSample[i].y;
  }
